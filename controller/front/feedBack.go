...
func MemberFeedBack(w http.ResponseWriter, r *http.Request) {
	body, err := ioutil.ReadAll(io.LimitReader(r.Body, 1024)) //io.LimitReader限制大小
	if err != nil {
		fmt.Println(err)
	}
	token :=r.Header.Get("Authorization")
	var data frontServices.MemberFeedBack
	...
	if(tokenStatus=="error"){
		...
		ResultCode,result :=frontServices.SetMemberFeedBack(data,member_no)
